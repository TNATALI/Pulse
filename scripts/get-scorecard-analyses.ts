...
import { loadGitHubAppCredentials, createInstallationOctokit } from '../backend/src/services/github-auth.js';
import { db } from '../backend/src/db/connection.js';
import { workspaces, repositories } from '../backend/src/db/schema/index.js';
...
async function main() {
  ...
  const repos = await db
    ...
    .from(repositories)
    .where(require('drizzle-orm').eq(repositories.workspaceId, workspace.id));
